...
- [x] Replace the flat "correct half of room = free, wrong half = flat `zone` penalty" cost with a continuous distance-to-whiteboard cost, reusing the exact relationship formulas: "front" behaves like a green (like) relationship with the whiteboard (cost scales with distance), "back" like a red (dislike) one (cost spikes as distance shrinks). Fixes reported under-weighting where "back" students settled near the middle instead of the far wall. `runOptimization`'s `roomHeight`/`frontIsTop` params are replaced by a single `whiteboardPos: {x,y} | null`, derived in `useStore.ts` from the whiteboard furniture's actual position; the now-unused `weights.zone` is removed.
  - Test: `solver.test.ts` - given 3 seats at increasing distance from the whiteboard, a "front" student is pulled to the nearest seat and a "back" student is pushed to the farthest seat (not merely "the correct half"); zonePreference is a no-op (doesn't throw) when there's no whiteboard in the room.
